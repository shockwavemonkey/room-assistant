...
4. Now let's get install room-assistant! Run `sudo npm i --global --unsafe-perm --verbose --foreground-scripts room-assistant`. You will see messages like the one shown below during the installation process. Don't worry about them - they're not errors!
